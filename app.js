...
function edit(id){let r=data.find(x=>x.id===id);if(!r)return;$("editId").value=id;["issue","date","line","application","machine","requestBy","pic","status","due","close","rca","ca","pa","verification","notes","lesson"].forEach(k=>$(k).value=r[{issue:"issue",date:"issueDate",line:"line",application:"application",machine:"machine",requestBy:"requestBy",pic:"pic",status:"status",due:"dueDate",close:"actualCloseDate",rca:"rootCause",ca:"correctiveAction",pa:"preventiveAction",verification:"verification",notes:"notes",lesson:"lessonLearned"}[k]]||"");$("failure").value=r.failureMode;category();$("category").value=r.category;attach=r.attachments||[];showAttach()}
